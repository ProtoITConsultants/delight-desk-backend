...
interface AddressChangeActionFailureResult {
  success: false;
...
export function buildAddressChangeFailureResult(
  state: AddressChangeWorkflowState,
  actionResult: ActionExecutionResult,
): AddressChangeActionFailureResult | null {
...
export function assessCustomerDistress(
  ...
  const matchedUrgencyKeywords = findMatchedKeywords(
    normalizedText,
    CUSTOMER_DISTRESS_URGENCY_KEYWORDS,
  );
