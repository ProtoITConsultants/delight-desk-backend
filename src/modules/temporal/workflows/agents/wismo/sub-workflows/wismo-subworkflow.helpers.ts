...
interface WismoActionFailureResult {
  success: false;
...
/**
 * Maps a failed workflow action result to the shared WISMO failure shape.
 * Preserves existing escalation/cancellation behavior for sub-workflows.
 */
export function buildWismoFailureResult(
  state: WismoWorkflowState,
  actionResult: ActionExecutionResult,
): WismoActionFailureResult | null {
